...
def make_pull_request(sandbox: Sandbox, args: Dict[str, Any]) -> str:
    ...
    gh_pr_cmd = f'gh pr create --base "{base_branch}" --head "{new_branch_name}" --title "{title}" --body "{body}"'.replace('`', '\\`')

    gh_pull_request_proc = sandbox.process.start_and_wait(cmd=gh_pr_cmd, cwd=REPO_DIRECTORY)
